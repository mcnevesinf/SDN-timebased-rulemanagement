...
class LearningSwitch (EventMixin):
  ...
  def __init__ (self, connection, transparent):
    ...
    #-------------------------------------------------------------
    #        AHTM - Control structures
    #-------------------------------------------------------------
    #Structure to control the flow duration
    self.AHTM_controlStruct = []
  ...
  def _handle_PacketIn (self, event):
    ...
    def AHTM_getFlowIndex( AHTM_flowData ):
        flowIndex = -1

        if len(self.AHTM_controlStruct) > 0:
            iterator = 0
            indexNotFound = True
            #print(self.knownFlows[0])
            while iterator < len(self.AHTM_controlStruct) and indexNotFound:
                if AHTM_flowData[0] == self.AHTM_controlStruct[iterator][0] and \
                   AHTM_flowData[1] == self.AHTM_controlStruct[iterator][1] and \
                   AHTM_flowData[2] == self.AHTM_controlStruct[iterator][2] and \
                   AHTM_flowData[3] == self.AHTM_controlStruct[iterator][3]:
                    flowIndex = iterator
                    indexNotFound = False
	...
	#IF applying AHTM THEN calculate specific hard timeout
	if PARAM_AHTMModule == "True":

	    #Update AHTM control structure and get flow duration
	    AHTM_flowDuration = 0
	    AHTM_flowData = getFlowData( packet )

	    if AHTM_flowData is not None:
		AHTM_flowIndex = AHTM_getFlowIndex( AHTM_flowData )

		FLOW_DURATION_POSITION = 4

		if AHTM_flowIndex == -1:
		    #First packet of the flow. Insert control data into the control structure
		    AHTM_controlData = []
								
		    for iterator in range(0,4):
			AHTM_controlData.append( AHTM_flowData[iterator] )

		    AHTM_controlData.append( int( time.time() ) )
		    self.AHTM_controlStruct.append( AHTM_controlData )
		    #print("AHTM Module: New flow. Time: "+str( int( time.time() ) ))

		    AHTM_flowDuration = 0
		else:
		    #print("AHTM Module: Flow index: "+str(AHTM_flowIndex))

		    AHTM_flowDuration = int( time.time() ) - self.AHTM_controlStruct[AHTM_flowIndex][FLOW_DURATION_POSITION]
		
		#print("AHTM Module: Flow duration - "+str( AHTM_flowDuration ))
	    ...
	    if flowTableOccupancyList[switchIndex] >= PARAM_tableSize:
	        #print("AHTM Module: Table full. Controller acting.")
		
		#Do not apply rule caching policy
		msg = of.ofp_packet_out()

	    else:
		#print("AHTM Module: Table not full. Hard timeout applied.")
		...
		#Set timeouts - idle and hard
		msg.idle_timeout = of.OFP_FLOW_PERMANENT
		#msg.hard_timeout = 30

		#Define timeout according to the traffic characteristics and the duration of the flow
		AHTM_timeout = int( random.lognormvariate(4, 1) ) #Flow duration follows a LOGNORMAL distribution in our experiments

		#IF draw timeout is less than the flow duration THEN keep this timeout
		#if AHTM_timeout <= AHTM_flowDuration:
		#    msg.hard_timeout = AHTM_timeout
		#else:
		#    msg.hard_timeout = AHTM_timeout - AHTM_flowDuration

		msg.hard_timeout = int(PARAM_hardTime)
		#print("AHTM Module: Hard timeout " + str(msg.hard_timeout))

        #ELIF applying SMART_TIME THEN calculate specific idle timeout
        elif PARAM_smartTimeModule == "True":
 	    #print("SMART TIME MODULE: acting!")	
	    ...
	    msg = of.ofp_flow_mod()
            ...
            if SMART_TIME_flowData is not None:            
                msg.idle_timeout = SMART_TIME_getIdleTime( SMART_TIME_flowData, event )  #>>>>>>>>>>>>>>>>> BUG HERE <<<<<<<<<<<<<<<<
            else:
                msg.idle_timeout = int(PARAM_idleTime) #The rule does not represent a flow

	    print("SMART TIME MODULE: Idle time - "+str(msg.idle_timeout))
	    ...
	    #print("STATIC TIMEOUT MODULE: acting!")
            ...
            if PARAM_markovModule == "False":
                ...
                try:
                    ...
                    print("MAIN MODULE: Error - Switch not found.")
								...
								#When Markov_deletion module is ON, full table mode is controlled by that module
                if flowTableOccupancyList[switchIndex] >= PARAM_tableSize:
...
def launch (transparent=False, idleTime = 10, hardTime=30, getStatInterval = 2, simulationTime=60, tableSize=50, 
            markovModule="True", markovChain="defaultChain.txt", deletionInterval=2.0, smartTimeModule="False",
            smartTimeMinIdleTimeout=1, smartTimeMaxIdleTimeout=60, smartTimeTableUtilization=95, 
	    smartTimeIncreaseFunction="EXPONENTIAL", AHTMModule="False"):
  ...
  #                  AHTM
  #--------------------------------------------	
  global PARAM_AHTMModule

  PARAM_AHTMModule = AHTMModule

  if PARAM_AHTMModule == "True":
    print("AHTM module is ON.")
  else:
    print("AHTM module is OFF.")
  ...
  if PARAM_smartTimeModule == "True":
      PARAM_smartTimeMinIdleTimeout = smartTimeMinIdleTimeout
      PARAM_smartTimeMaxIdleTimeout = smartTimeMaxIdleTimeout
